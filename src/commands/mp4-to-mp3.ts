...
export async function mp4ToMp3(opts: { input: string; output: string; concurrency: number }): Promise<void> {
	const { input, output, concurrency } = opts

	printHeader(`Converting MP4/M4A → MP3: ${input} → ${output}`)

	const files: string[] = []
	for await (const entry of walk(input, { includeDirs: false, exts: ['.mp4', '.m4a', '.m4v'] })) {
		files.push(entry.path)
	}

	if (files.length === 0) {
		printInfo('No MP4/M4A/M4V files found')
		return
	}
	...
	for (let i = 0; i < files.length; i += concurrency) {
		...
		await Promise.all(batch.map(async (f) => {
			...
			const dest = join(output, rel.replace(/\.(mp4|m4a|m4v)$/i, '.mp3'))
