...
export async function m4aToMp3(opts: { input: string; output: string; concurrency: number }): Promise<void> {
	const { input, output, concurrency } = opts

	printHeader(`Converting M4A → MP3: ${input} → ${output}`)

	const files: string[] = []
	for await (const entry of walk(input, { includeDirs: false, exts: ['.m4a'] })) {
		files.push(entry.path)
	}

	if (files.length === 0) {
		printInfo('No M4A files found')
		return
	}
	...
	for (let i = 0; i < files.length; i += concurrency) {
		...
		await Promise.all(batch.map(async (f) => {
			...
			const dest = join(output, rel.replace(/\.m4a$/i, '.mp3'))
