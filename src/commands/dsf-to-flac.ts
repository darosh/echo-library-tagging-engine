...
export async function dsfToFlac(opts: { input: string; output: string; concurrency: number }): Promise<void> {
	const { input, output, concurrency } = opts

	printHeader(`Converting DSF → FLAC: ${input} → ${output}`)

	const files: string[] = []
	for await (const entry of walk(input, { includeDirs: false, exts: ['.dsf'] })) {
		files.push(entry.path)
	}

	if (files.length === 0) {
		printInfo('No DSF files found')
		return
	}
	...
	for (let i = 0; i < files.length; i += concurrency) {
		...
		await Promise.all(batch.map(async (f) => {
			...
			const dest = join(output, rel.replace(/\.dsf$/i, '.flac'))
